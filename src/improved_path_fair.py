...
from src import experiment_recorder
...
def get_samoded_samples(lower_bound, upper_bound, sa_index, origin_x):
    global sa_space
    if sa_space is None:
        possible_sa = []
        for i in range(lower_bound, upper_bound + 1):
            possible_sa.append(i)
        sa_space = np.array(possible_sa)[:, np.newaxis]
    extended_x = np.tile(origin_x, (upper_bound - lower_bound + 1, 1))
    extended_x = np.concatenate([extended_x[:, :sa_index], sa_space, extended_x[:, sa_index + 1:]], axis=1)
    return extended_x
...
def check_for_error_condition(conf, sess, x, preds, t, sens):
    ...
    tuple_t = tuple(t)
    global check_cache
    if tuple_t in check_cache:
        return check_cache[tuple_t]

    samoded_x = get_samoded_samples(conf.input_bounds[sens - 1][0], conf.input_bounds[sens - 1][1], sens - 1, t)
    labels = model_argmax(sess, x, preds, samoded_x)
    value_cnt = np.unique(labels).shape[0]
    check_cache[tuple_t] = value_cnt > 1
    return value_cnt > 1
...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter, ReLU_name,
                     ...
                     global_dacay=0.1,
                     bias_alpha=0.6375, unbalance_check=None, data_use='train',
                     **kwargs):
    ...
    """
    if unbalance_check and data_preproc != 'random':
        raise Exception('Illegal unbalance check preprocess method')
    unbalance_check_list = None
    if unbalance_check:
        unbalance_check_list = []
    ...
    if data_use == 'full':
        ...
        raise Exception(f'Unknown param value "{data_use}" of data_use, can only be one of train/full')
    ...
    with tf.Graph().as_default():
        ...
        def evaluate_local(inp):
            ...
            temp = np.copy(inp.astype('int')).tolist()
        ...
        print(f"Size of training set:{len(X)}")
        if unbalance_check:
            global_input_size = int(len(X) * unbalance_check)
        else:
        ...
        for num in range(global_input_size):
            ...
            for iter in range(max_iter + 1):
                ...
                temp = np.copy(sample[0].astype('int')).tolist()
                ...
                if label != n_label and (tuple(temp) not in global_disc_inputs) and (
                    ...
                    cur_h, cur_m, cur_s = float_sec2hms(cur_time - start_time)
                    remain_time_multiple = (global_input_size - num - 1) / (num + 1)
                    remain_h, remain_m, remain_s = float_sec2hms(remain_time_multiple * (cur_time - start_time))
                    print("current time cost:%dh-%dm-%ds, remain time estimate:%dh-%dm-%ds" % (
                        cur_h, cur_m, cur_s, remain_h, remain_m, remain_s))
                    ...
                    if unbalance_check:
                        unbalance_check_list.append(len(local_disc_inputs))
                        print(f"#IDIP generated by current input:{len(local_disc_inputs)}")
                ...
                if iter == max_iter:
                    if unbalance_check:
                        unbalance_check_list.append(0)
                        print('0 gen')
                        print(f"#IDIP generated by current input:0")
                    break
        ...
        if unbalance_check:
            print(f"unbalance check list:{unbalance_check_list}")

        base_path = './output/' + kwargs['exp_name'] + '/'
        ...
        np.save(base_path + 'suc_value.npy', np.array(value_list))
